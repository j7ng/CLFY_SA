CREATE OR REPLACE TYPE sa."PCRF_TRANS_DET_LOW_PRTY_TYPE" IS OBJECT
(
  ...
  CONSTRUCTOR FUNCTION pcrf_trans_det_low_prty_type RETURN SELF AS RESULT,
  CONSTRUCTOR FUNCTION pcrf_trans_det_low_prty_type ( i_pcrf_transaction_id IN  NUMBER,
  ...
  MEMBER FUNCTION ins ( i_pcrf_transaction_id IN  NUMBER,
                        ...
                        i_future_ttl          IN  DATE,
                        i_redemption_date     IN  DATE,
                        i_offer_name          IN  VARCHAR2) RETURN pcrf_trans_det_low_prty_type,
  MEMBER FUNCTION upd RETURN pcrf_trans_det_low_prty_type,
...
/
CREATE OR REPLACE TYPE BODY sa."PCRF_TRANS_DET_LOW_PRTY_TYPE" AS

CONSTRUCTOR FUNCTION pcrf_trans_det_low_prty_type RETURN SELF AS RESULT AS
BEGIN
  -- TODO: Implementation required for FUNCTION pcrf_trans_det_low_prty_type.pcrf_trans_det_low_prty_type
  RETURN;
END pcrf_trans_det_low_prty_type;
...
BEGIN
  ...
  --
  SELECT pcrf_trans_det_low_prty_type ( objid               ,
                                        pcrf_transaction_id ,
                                        offer_id            ,
                                        ttl                 ,
                                        future_ttl          ,
...
MEMBER FUNCTION exist RETURN BOOLEAN IS
  ...
  pcrf  pcrf_trans_det_low_prty_type := pcrf_trans_det_low_prty_type ( i_pcrf_transaction_id => SELF.pcrf_transaction_id,
...
MEMBER FUNCTION ins ( i_pcrf_transaction_id IN  NUMBER,
                      ...
                      i_future_ttl          IN  DATE,
                      i_redemption_date     IN  DATE,
                      i_offer_name          IN  VARCHAR2) RETURN pcrf_trans_det_low_prty_type AS

  pcrf      pcrf_transaction_type := pcrf_transaction_type ( i_pcrf_transaction_id => i_pcrf_transaction_id);
  detail    pcrf_trans_det_low_prty_type := SELF;
...
MEMBER FUNCTION upd RETURN pcrf_trans_det_low_prty_type AS

  detail pcrf_trans_det_low_prty_type := SELF;

  dt     pcrf_trans_det_low_prty_type := pcrf_trans_det_low_prty_type ( i_pcrf_transaction_id => SELF.pcrf_transaction_id,
                                                                        i_offer_id            => SELF.offer_id,
                                                                        i_redemption_date     => SELF.redemption_date);
BEGIN
  -- TODO: Implementation for FUNCTION pcrf_trans_det_low_prty_type.update_pcrf_detail
...
MEMBER FUNCTION del ( i_pcrf_transaction_id IN  NUMBER,
  ...
  detail pcrf_trans_det_low_prty_type := pcrf_trans_det_low_prty_type ( i_pcrf_transaction_id => i_pcrf_transaction_id,
...
BEGIN
  ...
  DELETE x_pcrf_trans_detail_low_prty
