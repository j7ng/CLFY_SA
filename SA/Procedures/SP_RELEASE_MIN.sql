CREATE OR REPLACE PROCEDURE sa."SP_RELEASE_MIN"
IS
/******************************************************************************/
   ...
   IS
      SELECT *
       ...
       WHERE  warr_end_date <> to_date('01/01/1753','mm/dd/yyyy')
         ...
         AND x_part_inst_status = '11';


   pi_rec cur_pi%ROWTYPE;
/* get manufacturer */
   CURSOR cur_manuf (c_objid IN NUMBER)
   IS
...
/* get carrier id */
   CURSOR cur_cid (c_objid IN NUMBER)
   IS
      SELECT x_carrier_id
...
/* get site id from inv_bin for dealer id*/
   CURSOR cur_did (c_objid IN NUMBER)
   IS
      SELECT bin_name
...
/* get site_id for cust_id */
   CURSOR cur_si (c_objid IN NUMBER)
   IS
      SELECT site_id
...
BEGIN
   ...
   LOOP
      FETCH cur_pi INTO pi_rec;
      EXIT WHEN cur_pi%NOTFOUND;
      OPEN cur_cid (pi_rec.objid);
      FETCH cur_cid INTO cid_rec;
      CLOSE cur_cid;
      OPEN cur_manuf (pi_rec.objid);
      FETCH cur_manuf INTO manuf_rec;
      CLOSE cur_manuf;
      OPEN cur_did (pi_rec.objid);
      FETCH cur_did INTO did_rec;
      CLOSE cur_did;
      OPEN cur_si (pi_rec.objid);
      FETCH cur_si INTO si_rec;
      CLOSE cur_si;
...
--     concat('1',lpad(cid_rec.x_carrier_id,9,'0')),
      ...
      UPDATE table_part_inst
         SET x_part_inst_status = '18',
             status2x_code_table = 964
...
EXCEPTION
   ...
   THEN
      ...
      BEGIN
         ...
         sql_err := SQLERRM;
         file_handle :=
            UTL_FILE.fopen ('/f01/invfile', 'release_exceptions', 'A');
         UTL_FILE.putf (
            file_handle,
            '%s\n%s\n%s\n',
            TO_CHAR (SYSDATE, 'MM-DD-YYYY-HH-MI-SS'),
