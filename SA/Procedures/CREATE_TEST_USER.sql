CREATE OR REPLACE Procedure sa.Create_test_User (
...
Where  S_Class_Name = Upper('SYSTEM ADMINISTRATOR' );
--'CALL CENTER AGENT','APP - SUPERVISOR'; 'SYSTEM ADMINISTRATOR'
...
Into Found_Rec
From All_Users
Where Username = upper(Ip_Login_Name);
...
if c2%notfound then
  ...
  sql_stmt := 'grant connect to  '||Ip_Login_Name||' identified by abc123';
...
x_dashboard,x_allow_script,x_allow_roadside,employee2user,supp_person_off2site,emp_supvr2employee)
 values
(V_EMPLOYEE_OBJID,IP_FIRST_NAME,upper(IP_FIRST_NAME),IP_LAST_NAME,upper(IP_LAST_NAME),
...
V_TIME_BOMB_OBJID,IP_LOGIN_NAME, TO_DATE( '1/1/1753 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),
