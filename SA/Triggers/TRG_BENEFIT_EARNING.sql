CREATE OR REPLACE TRIGGER sa.TRG_BENEFIT_EARNING AFTER
  INSERT OR
  UPDATE ON sa.x_reward_benefit_earning REFERENCING OLD AS OLD NEW AS NEW FOR EACH ROW DECLARE l_column_name VARCHAR2(100);
  ...
  BEGIN
    ...
    IF INSERTING THEN
      ...
      WHERE TABLE_NAME= 'X_REWARD_BENEFIT_EARNING'
      AND OWNER       ='SA'
      )
      LOOP
        ...
        ELSIF l_column_name    ='BENEFITS_EARNED' THEN
          l_column_value   := :new.BENEFITS_EARNED;
        ...
        elsif l_column_name ='TRANSACTION_TYPE' THEN
          l_column_value   := :new.TRANSACTION_TYPE;
        END IF;
        INSERT
    ...
    ELSIF UPDATING THEN
      ...
      IF  l_column_name IN ('OBJID', 'START_DATE', 'END_DATE', 'BENEFITS_EARNED') THEN
      ...
      ln_objid := seq_x_reward_history.nextval;
       ...
       IF l_column_name = 'OBJID' THEN
          ...
          ln_objid_to_action := ln_objid;
      ELSIF (:OLD.BENEFITS_EARNED <> :NEW.BENEFITS_EARNED) THEN
        l_column_name          :='BENEFITS_EARNED';
        l_column_value         := :new.BENEFITS_EARNED;
    ...
    ElSIF DELETING THEN
     raise_application_error (-20002,SUBSTR('DELETE FROM X_REWARD_BENEFIT_EARNING TABLE IS NOT ALLOWED '||SQLERRM,1,255));
