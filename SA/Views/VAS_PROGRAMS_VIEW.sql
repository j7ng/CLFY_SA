CREATE OR REPLACE FORCE VIEW sa.vas_programs_view (vas_service_id,vas_name,product_id,vas_bus_org,vas_product_type,vas_category,vas_group_name,vas_description_english,vas_description_spanish,vas_is_active,vas_start_date,vas_end_date,vas_price,vas_vendor,vas_association,vas_recurring_days,service_days,grace_period,vas_sponsor,vas_tax_calculation,vas_type,vas_app_card,vas_card_class,part_class_objid,program_parameters_objid,auto_pay_program_objid,x_promotion_objid,offer_expiry,reenroll_allow_flag,show_due_before_days,proration_flag,direct_cancel_flag,transfer_on_upgrade_flag,transfer_on_replacement_flag,refund_on_upgrade_flag,refund_on_replacement_flag,refund_on_cancellation_flag,electronic_refund_days,mobile_plan_type,mobile_description,mobile_description2,mobile_description3,mobile_description4,ild_rates_url,available_for_lrp_flag,points_required_to_redeem,points_accrued_by_purchase,points_accrued_by_autorefill,eligible_devices) AS
SELECT vas_service_id,
          ...
          sa.get_param_value('VAS_ELIGIBLE_DEVICES') --Added for TW Web common standards
     FROM (SELECT sv.*,
                  lrp.AVAILABLE_FOR_LRP_FLAG,
                  lrp.POINTS_REQUIRED_TO_REDEEM,
                  lrp.POINTS_ACCRUED_BY_PURCHASE,
                  lrp.POINTS_ACCRUED_BY_AUTOREFILL
             FROM sa.vas_params_view sv, lrp_points_mv lrp
            WHERE sv.vas_service_id = lrp.vas_service_id) PIVOT (MAX (
                                                                    vas_param_value)
                                                          FOR vas_param_name
                                                          IN  ('VAS_APP_CARD',
                                                              'VAS_CARD_CLASS',
