CREATE OR REPLACE PACKAGE BODY sa."SERVICE_DEACT_CATCHUP"
AS
/********************************************************************************/
   ...
   /*  1.3     03/02/2011 PM    CR15325 For new Deact Reason - ST SIM EXCHANGE
   *****************************************************************************/
...
PROCEDURE DEACTIVATE_PAST_DUE(p_bus_org_id in varchar2) IS
--------------------------------------------------------------------
        ...
        OPEN currdeactcode ('AC VOIDED', 'LS');
         ...
         OPEN currdeactcode (( case when ip_deactreason in ('STOLEN', 'PASTDUE', 'SELL PHONE', 'NO NEED OF PHONE',
                                    ...
                                    when IP_DEACTREASON in ( 'NTN','REFURBISHED', 'PORT OUT', 'MINCHANGE',
                                                             'CHANGE OF ADDRESS', 'PORTED NO A/I', 'SENDCARRDEACT','ST SIM EXCHANGE',  -- CR15325 Pmistry 02/25/2011 added new deact reason 'ST SIM EXCHANGE'
          ...
          if cur_newesn%found then
            ...
            UPDATE TABLE_PART_INST
             ...
             WHERE part_to_esn2part_inst = rec_newesn.objid
               ...
               and x_domain = 'LINES'   --CR15325 PMistry 03/02/2011
...
-- if suspend and x_cancel_suspend = 1 then insert into x_suspendtocancel table
         if strdeacttype = 'Suspend' and rec_min.X_CANCEL_SUSPEND = 1 then
           insert into x_canceltosuspend(OBJID            ,
                                         X_STATUS          ,
                                         X_MIN,
      ...
      IF (rec_ph.x_technology = 'GSM') and (rec_ph.x_iccid IS NOT NULL) THEN
        OPEN currdeactcode (( case when ip_deactreason in ('UPGRADE','SIM DAMAGED','SIM DAMAGE','ACTIVE UPGRADE',
                                   ...
                                   when  IP_DEACTREASON in ('SIM EXCHANGE','CHANGE OF ADDRESS','CUSTOMER REQUESTED',
                                                            'SIM CHANGE', 'ST SIM EXCHANGE')    then -- CR15325 Pmistry 02/25/2011 added new deact reason 'ST SIM EXCHANGE'
                                     'SIM VOID'
                                   else
                                     'SIM RESERVED'
                               end), 'SIM');
