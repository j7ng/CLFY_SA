CREATE OR REPLACE PACKAGE BODY sa."SERVICE_DEACTIVATION_CODE_TDMA"
AS
/********************************************************************************/
   ...
   /* look for TDMA_TURNDOWN to put name of temporary table */
   /********************************************************************************/
...
-- 06/09/03
   ...
   IS
      ...
      ml, table_part_num pn,  TDMA_TURNDOWN  pd
      WHERE ca.x_carrier_id NOT IN (
      SELECT e.x_carrier_id  FROM x_excluded_pastduedeact e)
      ...
      IS
         ...
         'select 1 from dual
                                where exists( select 1
                                             ...
                                             ;
         sql2 VARCHAR2 (5000) := 'truncate table ';
         sql3 VARCHAR2 (5000) :=
         'insert into
                               select /*+ FULL(sp) PARALLEL(sp,10) */
                                ...
                                where sp.x_expire_dt BETWEEN TO_DATE (''02-JAN-1753'', ''dd-mon-yyyy'') AND trunc(SYSDATE)
                                  AND sp.part_status||'''' = ''Active'' and sp.x_service_id in (select esn from TDMA_TURNDOWN) ';
      BEGIN
         BEGIN
...
------------------------------------------------------------------------------------------------
   BEGIN
-- take this out  ic    check_x_deact_table;
      -------
      SELECT objid
      INTO v_user
      FROM table_user
...
-- 06/09/03
   BEGIN
      ...
      THEN
         ...
         RETURN 0;
   END;
END Service_Deactivation_Code_Tdma;
/