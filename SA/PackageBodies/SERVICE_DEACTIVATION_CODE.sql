CREATE OR REPLACE PACKAGE BODY sa."SERVICE_DEACTIVATION_CODE" AS
 --------------------------------------------------------------------------------------------
 --$RCSfile: SERVICE_DEACTIVATION_CODE.sql,v $
  ...
  --$ Correct action item logic for WFM brm notifications via POJO (service_deactivation_code).
  --$
...
BEGIN
   ...
   IF LTRIM(ip_esn) IS NULL THEN
     ...
     VALUES (op_returnmsg,
             ...
             'deactservice('|| ip_sourcesystem    ||','||
                               ...
                               op_return          ||','||
                               op_returnmsg       ||')',
                               ip_min,'service_deactivation_code.deactservice');
   ...
   IF tct_min.expire_subscriber_flag = 'Y' THEN
       sa.SERVICE_PROFILE_PKG.delete_subscriber (i_min              => ip_min,
                                                 ...
                                                 i_src_program_name => 'SERVICE_DEACTIVATION_CODE',
   ...
   sa.SERVICE_DEACTIVATION_CODE.create_call_trans (rec_ph.sitepartobjid
                                                  ,2
   ...
   sa.SERVICE_DEACTIVATION_CODE.remove_autopay_prc (rec_ph.part_serial_no
                                                   ,rec_ph.org_id
                                                   ,strrettemp);
...
WHEN OTHERS THEN
  ...
  INTO error_table
    ...
    (
      ...
      NULL,
      NULL,
      'service_deactivation_code.expirecdmasims'
    );
  COMMIT;
  op_result := 1;
