CREATE TABLE sa.x_rpt_invalid_redemption (
  ...
  CONSTRAINT x_rpt_invalid_redemption_u1 UNIQUE (card_smp) USING INDEX sa.x_rpt_invalid_redemption_indx1
);
ALTER TABLE sa.x_rpt_invalid_redemption ADD SUPPLEMENTAL LOG GROUP dmtsora924417445_0 (action_type, activation_date, activation_zipcode, call_trans_objid, card_access, card_number, card_smp, card_units, carrier_grp_add1, carrier_grp_add2, carrier_grp_address_objid, carrier_grp_city, carrier_grp_id, carrier_grp_name, carrier_grp_objid, carrier_id, carrier_mkt_add1, carrier_mkt_add2, carrier_mkt_address, carrier_mkt_city, carrier_mkt_name, carrier_mkt_state, carrier_mkt_zip, carrier_objid, cellnum, cellnum_account_num, cellnum_insert_date, esn, esn_po_number, promo_code, promo_objid, redemption_date, sourcesystem) ALWAYS;
ALTER TABLE sa.x_rpt_invalid_redemption ADD SUPPLEMENTAL LOG GROUP dmtsora924417445_1 (act_technology, card_dealer_id, card_dealer_name, card_dealer_objid, card_part_number, card_part_number_description, card_part_number_objid, carrier_grp_state, carrier_grp_zip, click_plan_id, cust_add1, cust_add2, cust_city, cust_first_name, cust_home_phone, cust_last_name, cust_state, cust_zip, dealer_objid, esn_dealer_id, esn_dealer_name, esn_part_number, esn_part_number_description, esn_part_number_objid, esn_technology, personality_id, redemption_type, user_first_name, user_last_name, user_login_name, user_objid, validated_date, valid_dealer) ALWAYS;
ALTER TABLE sa.x_rpt_invalid_redemption ADD SUPPLEMENTAL LOG GROUP dmtsora924417445_2 (sub_sourcesystem) ALWAYS;