CREATE OR REPLACE function sa.get_test_esn_old
                 (esn_part_number in varchar2)
                  return varchar2 is

cursor c1 is
select table_mod_level.objid,x_technology,X_PARAM_VALUE
from table_part_num, table_mod_level,table_x_part_class_values, table_x_part_class_params
...
begin
   ...
   if r1.x_technology = 'GSM' then
    SELECT GSM_SERIAL_NO
    into serial_no
    FROM DUAL;

  else
    SELECT CDMA_SERIAL_NO_SEQ.NEXTVAL
    into serial_no
    FROM DUAL;

    SELECT MEIDDECTOHEX(serial_no)
      INTO new_hex_meid
      FROM dual;
  end if;
  ...
  from dual;
    ...
    values (sa.seq('x_ota_features'),null,'Y','Y','N',null,piobjid,'32275',null,'Inactive','Completed',null,0,r1.X_PARAM_VALUE,'Y','Y','Y',null,null,null);

    commit;

    return to_char(serial_no);
