CREATE OR REPLACE function sa.create_active_esn_contact (
...
op_err_code     VARCHAR2(300);
op_err_msg      VARCHAR2(300);
--
...
IS
Begin
--  delete from table_part_inst where part_serial_no= ip_min and x_domain='LINES';
...
--CREATE CONTACT/SITE
  createcontact_prc(
   ...
   ,p_st              =>                     'FL'
   ,p_zip             =>                     '33122'
   ,p_email           =>                     upper(ip_esn||'@'||v_bus_org||'.Com')
   ,p_email_status    =>                     0
   ,p_roadside_status =>                     0
   ,p_no_name_flag    =>                     0
   ,p_no_phone_flag   =>                     0
   ...
   ,p_brand_name      =>                     v_bus_org
   ,p_do_not_email    =>                     0
   ,p_do_not_phone    =>                     0
   ,p_do_not_mail     =>                     0
   ,p_do_not_sms      =>                     0
   ,p_ssn             =>                     NULL
   ,p_dob             =>                     SYSDATE -(35*365)
   ,p_do_not_mobile_ads =>                     0
   ,p_contact_objid     =>                     l_contact_objid
   ,p_site_objid             =>                 l_site_objid
...
------------------------------------------------------------------
  IF NOT INSERT_LINE_REC (v_min_objid,
                         ...
                         v_esn_objid) THEN
         v_exception_id := 112;
         dbms_output.put_line('MIN:' ||op_min);
         dbms_output.put_line('ESN OBJ :' ||v_esn_objid);
...
-- insert table_site_part
 ...
 commit;
 update table_part_inst set LAST_TRANS_TIME=sysdate, WARR_END_DATE=trunc(v_expire_date)+30,X_PART_INST2SITE_PART =l_sitepart_objid,
 ...
 op_call_trans_objid      :=   l_call_trans_objid;
   ...
   commit;
     ...
     then
      ...
      values(l_sitepart_objid, 21, '0.04',null, sysdate);
      else 
            insert into  X_SERVICE_PLAN_SITE_PART (TABLE_SITE_PART_ID,  X_SERVICE_PLAN_ID ,  X_SWITCH_BASE_RATE,X_NEW_SERVICE_PLAN_ID , X_LAST_MODIFIED_DATE  )
      values(l_sitepart_objid, 82, '0.04',null, sysdate);
      end if;
      insert into X_SWITCHBASED_TRANSACTION   (OBJID ,X_SB_TRANS2X_CALL_TRANS,STATUS,X_TYPE,X_VALUE, EXP_DATE,RSID  )
      values(SEQU_X_SB_TRANSACTION.NEXTVAL,l_call_trans_objid,'Completed','A','0.04', trunc(sysdate)+30,'5050');
     ...
     update table_x_call_trans set x_reason='Activation', X_NEW_DUE_DATE= sysdate +30, x_total_units=0 where objid=l_call_trans_objid;
      commit;
     end if;
 return op_min;
end ;
