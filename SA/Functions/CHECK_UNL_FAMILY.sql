CREATE OR REPLACE FUNCTION sa."CHECK_UNL_FAMILY" (p_esn in varchar2, p_program_id in number)
return number
AS
/************************************************************************************************************************************/
...
cursor Esnxprogram_cur(v_program_id number, v_esn varchar2) is
 select pi.part_serial_no
  ...
  --  and pp.objid = v_program_id; -- unlimited net10 5801160 comment for change that included cross company
    and pp.x_program_name in ('UNLIMITED 30-DAY MONTHLY PLAN','UNLIMITED 30-DAY MONTHLY PLAN ILD');
